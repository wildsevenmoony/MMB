/*
 * Author: Moony
 * Defines the MMA fortification preset and object costs.
 */

class ACEX_Fortify_Presets {
    class MMA_fortificationSetSTAF {
        displayName = "STAF";
        objects[] = {
            {"EFM_mobile_barrier_A", 5},
            {"EFM_mobile_barrier_B", 5},
            {"EFM_mobile_dragons_teeth_small", 10},
            {"Land_Razorwire_F", 20},
            {"Land_BagFence_Long_F", 5},
            {"Land_BagFence_Round_F", 5},
            {"Land_BagFence_Corner_F", 3},
            {"Land_BagFence_End_F", 1},
            {"Land_BagFence_Short_F", 3},
            {"Land_BagFence_01_long_green_F", 5},
            {"Land_BagFence_01_round_green_F", 5},
            {"Land_BagFence_01_corner_green_F", 3},
            {"Land_BagFence_01_end_green_F", 1},
            {"Land_BagFence_01_short_green_F", 3},
            {"Land_BagBunker_Small_F", 50},
            {"Land_BagBunker_01_small_green_F", 50},
            {"Land_PortableLight_double_F", 5},
            {"Land_BarGate_F", 10},
            {"Land_HBarrier_01_line_5_green_F", 25},
            {"Land_HBarrier_01_line_3_green_F", 15},
            {"Land_HBarrier_01_line_1_green_F", 5},
            {"Land_HBarrier_5_F", 25},
            {"Land_HBarrier_3_F", 15},
            {"Land_HBarrier_1_F", 5}
        };
    };
};
