...
private _addCheckbox = {
    ...
    _buttonNo setVariable ["MMB_main_toggleControls", [_checkbox, _buttonNo, _buttonYes]];
    _buttonYes setVariable ["MMB_main_toggleControls", [_checkbox, _buttonNo, _buttonYes]];

    _buttonNo ctrlAddEventHandler ["ButtonClick", {
        params ["_buttonNo"];

        (_buttonNo getVariable ["MMB_main_toggleControls", [controlNull, controlNull, controlNull]]) params ["_checkbox", "_buttonNoStored", "_buttonYes"];
        if (!isNull _checkbox && {!isNull _buttonYes}) then {
            [_checkbox, _buttonNo, _buttonYes, false] call (_checkbox getVariable "MMB_main_setToggleVisuals");
        };
    }];
    ...
    _buttonYes ctrlAddEventHandler ["ButtonClick", {
        ...
        (_buttonYes getVariable ["MMB_main_toggleControls", [controlNull, controlNull, controlNull]]) params ["_checkbox", "_buttonNo", "_buttonYesStored"];
        if (!isNull _checkbox && {!isNull _buttonNo}) then {
            [_checkbox, _buttonNo, _buttonYes, true] call (_checkbox getVariable "MMB_main_setToggleVisuals");
        };
    }];

    _checkbox setVariable ["MMB_main_setToggleVisuals", _setToggleVisuals];
    _checkbox setVariable ["MMB_main_toggleControls", [_buttonNo, _buttonYes]];
...
private _addSlider = {
    ...
    private _labelControl = ["RscText", _labelIdc, [_labelTextX, _y, _rowWLabel, _rowH], _label] call _addControl;
    [_labelControl, _tooltip] call _applyTooltip;
    private _slider = ["MMB_main_RscXSliderH", _sliderIdc, [_fieldX, _y, _sliderW, _rowH]] call _addControl;
    ...
    _slider setVariable ["MMB_main_valueEdit", _valueEdit];
    _slider setVariable ["MMB_main_sliderRange", _range];
    _slider setVariable ["MMB_main_formatSliderValue", _formatSliderValue];
    _valueEdit setVariable ["MMB_main_valueSlider", _slider];

    _slider ctrlAddEventHandler ["SliderPosChanged", {
        params ["_slider", "_value"];

        private _valueEdit = _slider getVariable ["MMB_main_valueEdit", controlNull];
        if (!isNull _valueEdit) then {
            private _formatSliderValue = _slider getVariable "MMB_main_formatSliderValue";
    ...
    _valueEdit ctrlAddEventHandler ["KeyUp", {
        ...
        private _slider = _valueEdit getVariable ["MMB_main_valueSlider", controlNull];
        if (!isNull _slider) then {
            private _range = _slider getVariable ["MMB_main_sliderRange", [0, 100]];
