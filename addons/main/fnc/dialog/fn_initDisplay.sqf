#include "\z\mmb\addons\main\script_component.hpp"
...
uiNamespace setVariable [QGVAR(dynamicDialogDisplay), _display];
...
if (isNull _content) exitWith {
    diag_log format ["[MMB] %1: Missing content control IDC %2", QFUNC(initDisplay), IDC_MAIN_GROUP];
};
...
private _populateFunctionName = _display getVariable [QGVAR(populateFunction), ""];
if (_populateFunctionName isEqualTo "") exitWith {
    diag_log format ["[MMB] %1: Missing dialog populate function", QFUNC(initDisplay)];
};
...
if (!isNull _buttonOK) then {
    ...
    _buttonOK ctrlAddEventHandler ["ButtonClick", {
        ...
        [ctrlParent _control] call FUNC(confirmDisplay);
    }];
};
...
if (!isNull _buttonExport) then {
    _buttonExport ctrlAddEventHandler ["ButtonClick", {
        ...
        [ctrlParent _control] call FUNC(exportDisplayValues);
    }];
};
...
if (!isNull _buttonImport) then {
    _buttonImport ctrlAddEventHandler ["ButtonClick", {
        ...
        [ctrlParent _control] call FUNC(importDisplayValues);
    }];
};

// Center and resize the complete frame after content exists.
[_display] call FUNC(initDisplayPositioning);
