...

{
    ...
    if (isNull _control) then {
        diag_log format ["[MMB] %1: Missing registered field '%2' with IDC %3", QFUNC(collectDisplayValues), _label, _idc];
    } else {
        private _value = switch (_type) do {
            ...
            default {
                diag_log format ["[MMB] %1: Unknown field type '%2' for '%3'", QFUNC(collectDisplayValues), _type, _label];
                nil
            };
        };
